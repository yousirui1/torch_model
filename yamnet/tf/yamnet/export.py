...
class YAMNet(tf.Module):
  ...
  def __call__(self, waveform):
    if self.params.tflite_ouput == 2:
      predictions, predictions_1, embeddings, log_mel_spectrogram = self._yamnet(waveform)
...
def make_tflite_export(weights_path, export_dir, n_class, tflite_ouput):
  if os.path.exists(export_dir):
  ...
  params = yamnet_params.Params(tflite_compatible=True, num_classes = n_class, classifier_activation = 'softmax', tflite_ouput = tflite_ouput)
  yamnet = YAMNet(weights_path, params)
  #check_model(yamnet, yamnet.class_map_path(), params)
  log('Done')
